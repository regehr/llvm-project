...
def supports_required_passes(opt_bin: Path) -> bool:
    ...
    def has_pass(pass_name: str) -> bool:
        for line in proc.stdout.splitlines():
            normalized = line.strip()
            if normalized == pass_name or normalized.startswith(f"{pass_name}<"):
                return True
        return False

    # Some LLVM builds expose parameterized passes like `instcount<pre-opt>`
    # instead of a bare `instcount` entry in `--print-passes`.
    return has_pass("instcount") and has_pass("width-opt")
