...
; CHECK: %r = select i1 %c, i32 %zx, i32 42
; CHECK-NOT: select i1 %c, i8 %x, i8 42
; CHECK: ret i32 %r
...
; CHECK: %r = select <4 x i1> %c, <4 x i32> %zx, <4 x i32> splat (i32 42)
; CHECK-NOT: select <4 x i1> %c, <4 x i8> %x, <4 x i8> splat (i8 42)
; CHECK: ret <4 x i32> %r
